...
$block_id     = ! empty( $block['anchor'] ) ? sanitize_title( $block['anchor'] ) : 'block-industries-grid-' . $block['id'];
$block_class  = 'block-industries-grid';
...
<div id="<?php echo esc_attr( $block_id ); ?>" class="<?= esc_attr( $block_class )?> wp-block alignwide mt-10 mb-20 lg:mt-20 lg:mb-28 px-5">
	<div class="container">
		<?php
		$query_args = array(
			'post_type'      => 'industry', // Change 'post' to your custom post type if needed
			'posts_per_page' => -1,
			'order'          => 'DESC',
		);
